...
"Tipo_encuentro es de tipo TEXT. Los posibles valores son: Atención primaria, Hospital, Urgencia, Hospicio, Casa y Virtual."\
...
""
pregunta = "Pacientes mujeres que tengan alergia al polen y sean mayores de 20 años"
...
datos = abrirbasededatos(query)
# Usamos un set para extraer solo los IDs únicos
pacientes_unicos = list({fila[0] for fila in datos})
...
pdf.cell(0, 10, "Informe del Cohorte Médico", ln=1, align="C")
pdf.set_font("Arial", "", 12)
pdf.cell(0, 10, f"Fecha y hora de creación: {now}", ln=1, align="C")
pdf.ln(10)

# Iterar sobre cada paciente y agregar su sección en el mismo PDF
for paciente_id in pacientes_unicos:
    pdf.add_page()
    # Encabezado para cada paciente
    pdf.set_font("Arial", "B", 14)
    pdf.cell(0, 10, f"Paciente: {paciente_id}", ln=1)
    pdf.ln(5)
    ...
    pdf.set_font("Arial", "", 12)
    cursor.execute("SELECT PacienteID, Fecha_nacimiento, Fecha_muerte, Raza, Genero, Provincia, Edad FROM Pacientes WHERE PacienteID = ?", (paciente_id,))
    info = cursor.fetchone()
    if info:
        pdf.cell(0, 8, f"PacienteID: {info[0]}", ln=1)
        pdf.cell(0, 8, f"Fecha de Nacimiento: {info[1]}", ln=1)
        pdf.cell(0, 8, f"Fecha de Muerte: {info[2] if info[2] is not None else 'Vivo'}", ln=1)
        pdf.cell(0, 8, f"Raza: {info[3]}", ln=1)
        pdf.cell(0, 8, f"Género: {info[4]}", ln=1)
        pdf.cell(0, 8, f"Provincia: {info[5]}", ln=1)
        pdf.cell(0, 8, f"Edad: {info[6]}", ln=1)
    else:
        pdf.cell(0, 8, "No se encontró información general.", ln=1)
    pdf.ln(5)
    
    # Sección: Condiciones
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "Condiciones:", ln=1)
    pdf.set_font("Arial", "", 12)
    cursor.execute("SELECT Fecha_inicio, Fecha_fin, Codigo_SNOMED, Descripcion FROM Condiciones WHERE PacienteID = ?", (paciente_id,))
    condiciones = cursor.fetchall()
    if condiciones:
        for cond in condiciones:
            fecha_fin = cond[1] if cond[1] is not None else "Activo"
            pdf.multi_cell(0, 8, f"Condición: {cond[3]} (Código SNOMED: {cond[2]})\nInicio: {cond[0]} - Fin: {fecha_fin}\n")
            pdf.ln(1)
    else:
        pdf.cell(0, 8, "No se encontraron condiciones.", ln=1)
    pdf.ln(5)
    
    # Sección: Alergias
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "Alergias:", ln=1)
    pdf.set_font("Arial", "", 12)
    cursor.execute("SELECT Fecha_diagnostico, Codigo_SNOMED, Descripcion FROM Alergias WHERE PacienteID = ?", (paciente_id,))
    alergias = cursor.fetchall()
    if alergias:
        for al in alergias:
            pdf.multi_cell(0, 8, f"Alergia: {al[2]} (Código SNOMED: {al[1]})\nFecha diagnóstico: {al[0]}\n")
            pdf.ln(1)
    else:
        pdf.cell(0, 8, "No se encontraron alergias.", ln=1)
    pdf.ln(5)
    
    # Sección: Encuentros
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "Encuentros:", ln=1)
    pdf.set_font("Arial", "", 12)
    cursor.execute("SELECT EncuentroID, Fecha_inicio, Fecha_fin, Codigo_encuentro, Tipo_encuentro, Descripcion FROM Encuentros WHERE PacienteID = ?", (paciente_id,))
    encuentros = cursor.fetchall()
    if encuentros:
        for enc in encuentros:
            pdf.multi_cell(0, 8, f"EncuentroID: {enc[0]}\nTipo: {enc[4]}\nFecha inicio: {enc[1]}\nFecha fin: {enc[2]}\nDescripción: {enc[5]}\n")
            pdf.ln(1)
    else:
        pdf.cell(0, 8, "No se encontraron encuentros.", ln=1)
    pdf.ln(5)
    
    # Sección: Inmunizaciones
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "Inmunizaciones:", ln=1)
    pdf.set_font("Arial", "", 12)
    cursor.execute("SELECT Fecha, Codigo, Descripcion FROM Inmunizaciones WHERE PacienteID = ?", (paciente_id,))
    inmunizaciones = cursor.fetchall()
    if inmunizaciones:
        for inm in inmunizaciones:
            pdf.multi_cell(0, 8, f"Inmunización: {inm[2]} (Código: {inm[1]})\nFecha: {inm[0]}\n")
            pdf.ln(1)
    else:
        pdf.cell(0, 8, "No se encontraron inmunizaciones.", ln=1)
    pdf.ln(5)
    
    # Sección: Medicación
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "Medicación:", ln=1)
    pdf.set_font("Arial", "", 12)
    cursor.execute("""SELECT Fecha_inicio, Fecha_fin, Codigo, Via_de_administracion, Frecuencia, Descripcion, Dosis 
                      FROM Medicacion WHERE PacienteID = ?""", (paciente_id,))
    medicacion = cursor.fetchall()
    if medicacion:
        for med in medicacion:
            pdf.multi_cell(0, 8, f"Medicación: {med[5]} (Código: {med[2]})\nDesde: {med[0]} - Hasta: {med[1]}\nVía: {med[3]}\nFrecuencia: {med[4]}\nDosis: {med[6]}\n")
            pdf.ln(1)
    else:
        pdf.cell(0, 8, "No se encontró información de medicación.", ln=1)
    pdf.ln(5)
    
    # Sección: Procedimientos
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "Procedimientos:", ln=1)
    pdf.set_font("Arial", "", 12)
    cursor.execute("""SELECT Fecha_inicio, Fecha_fin, Codigo_SNOMED, Descripcion, Razon_descripcion 
                      FROM Procedimientos WHERE PacienteID = ?""", (paciente_id,))
    procedimientos = cursor.fetchall()
    if procedimientos:
        for proc in procedimientos:
            pdf.multi_cell(0, 8, f"Procedimiento: {proc[3]} (Código SNOMED: {proc[2]})\nDesde: {proc[0]} - Hasta: {proc[1]}\nRazón: {proc[4]}\n")
            pdf.ln(1)
    else:
        pdf.cell(0, 8, "No se encontraron procedimientos.", ln=1)
    pdf.ln(5)

# Guardar el PDF completo
output_filename = "Informe_Cohorte_Medico.pdf"
pdf.output(output_filename)
print(f"PDF completo creado: {output_filename}")
